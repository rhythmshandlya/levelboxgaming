  ...
  return (
    <>
      ...
      <div className="lg:mx-[200px] lg:my-[100px] mx-[5px] my-[10px]">
        ...
        <h1 className="text-center text-3xl lg:m-20 my-2">{news?.title}</h1>
        {news?.para?.map((element, index) => {
          if (isGroup(element)) {
            const s = element.substring(7);
            const arr = s.split("|");
            return (
              <div className="flex scroll-x-auto overflow-scroll hide-scrollbar">
                {arr.length == 3 &&
                  arr.map((image) => {
                    return (
                      <img
                        src={image}
                        className="rounded-xl mx-auto my-10 w-[400px]"
                      ></img>
                    );
                  })}
                {arr.length == 2 &&
                  arr.map((image) => {
                    return (
                      <img
                        src={image}
                        className="rounded-xl mx-auto my-10 w-[600px]"
                      ></img>
                    );
                  })}
                {arr.length == 1 &&
                  arr.map((image) => {
                    return (
                      <img
                        src={image}
                        className="rounded-xl mx-auto my-10 w-[700px]"
                      ></img>
                    );
                  })}
              </div>
